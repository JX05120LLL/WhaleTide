    ...
    <el-card class="box-card">
      <template #header>
        <div class="card-header">
          <span>收货地址管理</span>
          <el-button type="primary" @click="showAddressDialog(null)">添加新地址</el-button>
        </div>
      </template>
      ...
      <div v-if="addressList.length > 0" class="address-list">
        ...
        >
          ...
          <div class="address-actions">
            ...
            <el-divider direction="vertical"></el-divider>
            <el-button v-if="!address.isDefault" type="text" @click="setDefault(address.id)">设为默认</el-button>
          </div>
        </el-card>
      </div>
  ...
  setup() {
    ...
    const showAddressDialog = (address) => {
      ...
      if (address) {
        ...
        if (address.province && address.city && address.district) {
          // 这里需要根据地址找到对应的区域代码，实际项目中可能需要更复杂的逻辑
          // 简化处理：假设已经设置好
    ...
    const handleDelete = (id) => {
      ElMessageBox.confirm('确定要删除该地址吗？', '提示', {
        ...
        try {
          const res = await http.delete(`/address/delete/${id}`);
          if (res.code === 200) {
    ...
    const setDefault = async (id) => {
      try {
        const res = await http.put(`/address/default/${id}`);
        if (res.code === 200) {
