				...
				fetchCartList().then(response => {
					...
					if (!Array.isArray(list)) {
						...
						if (response.data && response.data.cartItems && Array.isArray(response.data.cartItems)) {
							list = response.data.cartItems;
							console.log('从response.data.cartItems获取购物车数据成功');
						} else {
							console.error('购物车数据不是数组');
							this.empty = true;
							return;
						}
					}
					
					console.log('购物车原始数据:', list);
